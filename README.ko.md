...
룰이든 스킬이든 슬래시 커맨드든 에이전트든 — AI 코딩 도구는 매 프롬프트마다 전부 로딩합니다. 5개면 괜찮지만, 50개 넘으면 커밋 메시지 쓰는데 React 베스트 프랙티스까지 같이 로딩됩니다.

그래서 대부분 자기검열을 합니다. 토큰 낭비가 걱정되어 룰이나 스킬을 적게 설치하고, 유용한 컨텍스트를 놓치게 됩니다.
